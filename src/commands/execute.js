...
exports.run = async (client, interaction) => {
	...
	const response = await fetch(`http://${process.env.ARBITER_URL}/game/execute/${id}?key=${process.env.ARBITER_KEY}`, {
		method: "POST",
		headers: { "content-type": "application/json" },
		body: JSON.stringify({ script }),
	}).catch((_) => _)

	if (response?.message) return interaction.createMessage("Arbiter or game is not running")
